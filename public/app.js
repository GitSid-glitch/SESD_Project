...
async function loadWorkspace() {
    ...
    const sprintEntries = await Promise.all(projects.map(async (project) => {
...
function renderMemberCollaborators() {
    ...
    const cards = Array.from(collaboratorMap.values()).map((member) => `
      <article class="user-card">
        <div class="user-row">
          <div>
...
function renderSprintManagementCards() {
    const cards = state.projects.flatMap((project) => (state.sprintsByProject[project.id] || []).map((sprint) => `
        <article class="list-card">
...
function syncTaskProjectContext(projectId) {
    ...
    elements.taskAssigneeSelect.innerHTML = [`<option value="">Unassigned</option>`]
        .concat((project?.members || []).map((member) => `<option value="${member.id}">${escapeHtml(member.name)} (${escapeHtml(member.role || "Member")})</option>`))
        .join("");
}
